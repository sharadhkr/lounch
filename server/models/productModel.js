...
const productSchema = new mongoose.Schema(
  {
    ...
    isCashOnDeliveryAvailable: {
      type: Boolean,
      default: false, // Default to false (COD not available unless specified)
    },
    onlinePaymentPercentage: {
      type: Number,
      default: 100, // Default to 100% online payment
      min: [0, 'Online payment percentage cannot be negative'],
      max: [100, 'Online payment percentage cannot exceed 100'],
      validate: {
        validator: function (value) {
          // If COD is available, allow any percentage (0-100); if not, must be 100
